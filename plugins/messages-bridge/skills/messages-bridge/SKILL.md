...
Treat the six read tools as read-only. Attachments must belong to the selected direct or group conversation. Unsupported image formats are returned as locally converted JPEGs. PDFs, videos, and other non-inline files up to 20 MB retain their original resource bytes and may also include a JPEG preview; larger previewable files return metadata and the preview without embedding the original. The two send tools can create external side effects but cannot edit, delete, run arbitrary SQL, or access arbitrary file paths.
